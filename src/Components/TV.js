...
class TV extends React.Component {
  ...
  componentDidMount() {
    ...
    this.db.collection("TV").onSnapshot((snapshot) => {
      const TV = snapshot.docs.map((doc) => {
        const data = doc.data();
        data["id"] = doc.id;
        return data;
      });
      this.setState({ TV: TV, loading: false });
    });
  }
  ...
  handleAdd = (TV) => {
    this.db.collection("cart").add(TV);
  };
  render() {
    const { TV, loading } = this.state;
    ...
    return (
      <div>
        ...
        <TVContainer TV={TV} addToCart={this.handleAdd} />
             {/*footer*/}
