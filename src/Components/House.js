...
class House extends React.Component {
  ...
  componentDidMount() {
    ...
    this.db.collection("house").onSnapshot((snapshot) => {
      const house = snapshot.docs.map((doc) => {
        const data = doc.data();
        data["id"] = doc.id;
        return data;
      });
      this.setState({ house: house, loading: false });
    });
  }
  ...
  handleAdd = (house) => {
    this.db.collection("cart").add(house);
  };
  render() {
    const { house, loading } = this.state;
    ...
    return (
      <div>
        ...
        <HouseContainer house={house} addToCart={this.handleAdd} />
        {/*footer*/}
