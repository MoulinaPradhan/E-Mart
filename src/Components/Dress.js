...
class Dress extends React.Component {
  ...
  componentDidMount() {
    ...
    this.db.collection("dress").onSnapshot((snapshot) => {
      const dress = snapshot.docs.map((doc) => {
        const data = doc.data();
        data["id"] = doc.id;
        return data;
      });
      this.setState({ dress: dress, loading: false });
    });
  }
  ...
  handleAdd = (dress) => {
    this.db.collection("cart").add(dress);
  };
  render() {
    const { dress, loading } = this.state;
    ...
    return (
      <div>
        ...
        <DressContainer dress={dress} addToCart={this.handleAdd} />
         {/*footer*/}
