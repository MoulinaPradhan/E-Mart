...
import BoysContainer from "./BoysContainer";
...
class Boys extends React.Component {
  ...
  componentDidMount() {
    ...
    this.db.collection("kidsboys").onSnapshot((snapshot) => {
      const kidsboys = snapshot.docs.map((doc) => {
        const data = doc.data();
        data["id"] = doc.id;
        return data;
      });
      this.setState({ kidsboys: kidsboys, loading: false });
    });
  }
  ...
  handleAdd = (kidsboys) => {
    this.db.collection("cart").add(kidsboys);
  };
  render() {
    const { kidsboys, loading } = this.state;
    ...
    return (
      <div>
        ...
        <BoysContainer kidsboys={kidsboys} addToCart={this.handleAdd} />
             {/*footer*/}
