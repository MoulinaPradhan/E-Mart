...
import MenPantContainer from "./MenPantContainer";
import Footer from "./Footer";
import * as firebase from "firebase";

import "bootstrap/dist/css/bootstrap.min.css";

class MenPant extends React.Component {
  constructor() {
    super();
    this.state = {
      pant: [],
  ...
  componentDidMount() {
    ...
    this.db.collection("pant").onSnapshot((snapshot) => {
      const pant = snapshot.docs.map((doc) => {
        const data = doc.data();
        data["id"] = doc.id;
        return data;
      });
      this.setState({ pant: pant, loading: false });
    });
  }
  ...
  handleAdd = (pant) => {
    this.db.collection("cart").add(pant);
  };
  render() {
    const { pant, loading } = this.state;
    ...
    return (
      <div>
        ...
        <MenPantContainer pant={pant} addToCart={this.handleAdd} />
   {/*footer*/}
